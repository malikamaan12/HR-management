...
export default function AttendanceOperations() {
    ...
    return <div className="space-y-6"><div className="flex flex-wrap justify-between gap-3"><h1 className="text-2xl font-bold">Attendance</h1>{['admin', 'super_admin'].includes(user?.role || '') && <a href="/hr-rules" className="underline text-primary">Configure employee calendars</a>}</div><Section title="My time clock"><QueryError error={current.error || location.error || clockError}/><p className="text-sm">{location.data?.policy.config.required ? 'Location is checked at clock-in and clock-out. Allow location access on this device.' : 'GPS enforcement is awaiting location setup or is disabled by an administrator.'} {locating ? 'Reading location…' : ''}</p><p>{clocked ? 'Clocked in since ' + new Date(current.data.checkIn).toLocaleString() : current.data?.checkOut ? 'Completed at ' + new Date(current.data.checkOut).toLocaleString() : 'Ready to clock in'}</p><div className="flex flex-wrap gap-3">{[['in', 'Clock in', !clocked && !current.data?.checkOut], ['break_start', 'Start break', clocked && !onBreak], ['break_end', 'End break', onBreak], ['out', 'Clock out', clocked]].map(([key, label, enabled]) => <Button key={String(key)} disabled={!enabled || action.isPending || locating || !location.data} onClick={() => clock(String(key))}>{label}</Button>)}</div></Section>
 <AttendanceLocationManagement/><AttendanceSupervisorQueue from={from} to={to}/><Section title="Schedule and recorded time"><div className="grid gap-4 md:grid-cols-2"><Field label="Employee"><select className={fieldClass} value={employee} onChange={e => setEmployee(e.target.value)}><option value="">Select employee</option>{options.data?.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select></Field><Field label="Attendance date"><input className={fieldClass} type="date" value={date} onChange={e => setDate(e.target.value)}/></Field></div><QueryError error={options.error || day.error}/>{day.data && <><p>{day.data.policy.timezone} · Scheduled: {day.data.plannedMinutes} minutes · Recorded work: {day.data.record?.totalWorkHours ?? '—'} minutes · Approval: {day.data.record?.approvalStatus || 'not required'} · Difference: {day.data.varianceMinutes ?? '—'} minutes{day.data.holiday ? ' · Holiday: ' + day.data.holiday : ''}</p><Table headers={['Team', 'Role', 'Start', 'End']} rows={day.data.roster.map((r: any) => [r.team, r.role, new Date(r.startAt).toLocaleString(undefined, { timeZone: r.timezone }), new Date(r.endAt).toLocaleString(undefined, { timeZone: r.timezone })])}/><a className="text-primary underline" href="/workforce">View workforce assignments</a></>}</Section>
 ...
 <Section title="Attendance report"><div className="flex flex-wrap gap-3"><Field label="From"><input className={fieldClass} type="date" value={from} onChange={e => setFrom(e.target.value)}/></Field><Field label="To"><input className={fieldClass} type="date" value={to} onChange={e => setTo(e.target.value)}/></Field><Button variant="outline" disabled={!reports.data?.details.length} onClick={() => downloadCsv('attendance.csv', [['Employee', 'Date', 'Status', 'Work minutes', 'Break minutes', 'Supervisor approval'], ...(reports.data?.details || []).map(r => [r.employeeName, r.date, r.status, r.totalWorkHours, r.totalBreakMinutes, r.approvalStatus])])}>Export CSV</Button></div><QueryError error={reports.error}/><Table headers={['Employee', 'Date', 'Status', 'Work minutes', 'Break minutes', 'Supervisor approval']} rows={(reports.data?.details || []).map(r => [r.employeeName, r.date, r.status, r.totalWorkHours ?? '—', r.totalBreakMinutes, r.approvalStatus])}/></Section></div>;
}
