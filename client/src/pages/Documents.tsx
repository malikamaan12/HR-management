...
export default function Documents(){
 ...
 <Tabs defaultValue="register"><TabsList><TabsTrigger value="register">Document register</TabsTrigger><TabsTrigger value="requests">Renewal requests</TabsTrigger><TabsTrigger value="policy">Approval rules</TabsTrigger></TabsList>
 ...
 <div className="grid gap-3 md:grid-cols-3"><label>Employee, number or type<Input aria-label="Search documents" placeholder="Search documents" value={search} maxLength={100} onChange={e=>setSearch(e.target.value)}/></label><label>Document type<Input placeholder="Exact type, e.g. Passport" value={type} maxLength={100} onChange={e=>setType(e.target.value)}/></label><label>Status<select className="block w-full border rounded p-2" value={status} onChange={e=>{setStatus(e.target.value);setPage(1);}}>{[['all','All documents'],['valid','Valid'],['expiring_soon','Expiring soon'],['expired','Expired']].map(([value,label])=><option key={value} value={value}>{label}{query.data?' ('+query.data.counts[value]+')':''}</option>)}</select></label></div>
