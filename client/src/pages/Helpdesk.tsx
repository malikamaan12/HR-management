...
export default function Helpdesk(){
  ...
  return <div className="space-y-6"><header className="flex flex-wrap items-start justify-between gap-4"><div className="flex items-start gap-4"><span className="rounded-2xl bg-primary/10 p-3 text-primary"><LifeBuoy className="h-7 w-7"/></span><div><h1 className="text-3xl font-semibold break-words">{workspace.title}</h1>{workspace.introduction&&<p className="mt-2 max-w-2xl whitespace-pre-wrap text-muted-foreground">{workspace.introduction}</p>}</div></div><Button onClick={()=>startRequest()}><Plus className="mr-2 h-4 w-4"/>New request</Button></header>
...
function NewCase({open,close,config,created,initialCategory}:{open:boolean;close:()=>void;config:HelpdeskConfig;created:(id:number)=>void;initialCategory:string}){
  ...
  return <Dialog open={open} onOpenChange={value=>{if(!value&&!mutation.isPending)close();}}><DialogContent className="max-h-[90vh] overflow-auto"><DialogHeader><DialogTitle>New HR request</DialogTitle><DialogDescription>{config.workspace.requestGuidance||config.workspace.title}</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={submit}><fieldset disabled={mutation.isPending} className="space-y-4">
    <Field label="Subject"><Input value={title} onChange={e=>setTitle(e.target.value)} minLength={4} maxLength={160} required autoFocus/></Field><Field label="Category"><select className={selectClass} required value={category} onChange={e=>setCategory(e.target.value as typeof category)}><option value="">Choose a category...</option>{config.workspace.categories.filter(c=>c.enabled).map(c=><option key={c.id} value={c.id}>{c.label}</option>)}</select></Field>
