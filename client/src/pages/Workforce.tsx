...
export default function Workforce() {
  ...
  const open=(kind:Modal)=>{setRequiredSkills([]);setForm({timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,kind:'event',headcount:'1',breakMinutes:'0',permission:'view',siteId:String(home.data?.sites[0]?.id||'')});setModal(kind);};
  async function save(event:FormEvent) {
    ...
    try {
      if(modal==='site') {url+='sites';body={name:form.name,timezone:form.timezone};}
        ...
        else {url+=`teams/${teamId}/shifts`;body={...dates,role:form.role,station:form.station||'',headcount:Number(form.headcount),breakMinutes:Number(form.breakMinutes),requiredSkills};}
      }
...
function ShiftCard({shift,zone,skills,members,busy,offer,cancel}:{shift:ShiftView;zone:string;skills:WorkforceSkill[];members:WorkforceDashboard['members'];busy:boolean;offer:(employeeId:number)=>void;cancel:(id:number,name:string)=>void}) {
