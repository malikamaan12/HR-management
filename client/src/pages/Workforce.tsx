import {Availability,Coverage,ShiftOperations} from '@/components/workforce/Operations';
...
type Modal='site'|'team'|'member'|'grant'|'shift'|'skills'|null;
export default function Workforce() {
  ...
  const [date,setDate]=useState(()=>{const value=new URLSearchParams(window.location.search).get('date');return value&&/^\d{4}-\d{2}-\d{2}$/.test(value)&&!Number.isNaN(Date.parse(value))?value:new Date().toISOString().slice(0,10);});
  const [requiredSkills,setRequiredSkills]=useState<number[]>([]);
  const [qualificationEmployee,setQualificationEmployee]=useState<{id:number;name:string}|null>(null);
  const catalogue=useQuery<WorkforceSkill[]>({queryKey:['/api/workforce/skills',{teamId}],enabled:modal==='shift'||modal==='skills'});
  ...
  useEffect(()=>{const id=new URLSearchParams(window.location.search).get('shiftId');if(id&&dashboard.data)document.getElementById(`shift-${id}`)?.scrollIntoView({behavior:'smooth'});},[dashboard.isLoading,teamId,date]);
  ...
  const open=(kind:Modal)=>{setRequestKey(crypto.randomUUID());setRequiredSkills([]);setForm({timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,kind:'event',count:'1',intervalDays:'7',headcount:'1',breakMinutes:'0',permission:'view',siteId:String(home.data?.sites[0]?.id||'')});setModal(kind);};
  async function save(event:FormEvent) {
    event.preventDefault(); if(modal==='skills')return; let url='/api/workforce/',body:unknown;
    try {
      if(modal==='site') {url+='sites';body={name:form.name,timezone:form.timezone};}
        ...
        else {const shift={...dates,role:form.role,station:form.station||'',headcount:Number(form.headcount),breakMinutes:Number(form.breakMinutes),requiredSkills};const count=Number(form.count||1);url+=`teams/${teamId}/${count>1?'series':'shifts'}`;body=count>1?{shift,count,intervalDays:Number(form.intervalDays),requestKey}:shift;}
      }
  ...
  const futureShifts=shifts.filter(s=>!s.cancelledAt&&new Date(s.endAt)>new Date());
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue={home.data?.teams.length || home.data?.isAdmin?'team':'mine'}>
      ...
      <TabsContent value="team" className="space-y-5 pt-3">
        ...
        {!teamId?<Card><CardContent className="py-10"><Users className="mb-3 h-8 w-8 text-primary"/><h2 className="text-lg font-semibold">{home.data?.isAdmin?'Set up your first operational team':'No team-lead access assigned'}</h2><p className="mt-2 text-muted-foreground">{home.data?.isAdmin?'Add a site and its time zone, create a team, then add employees and dated lead access.':'HR can grant you access to a team for specific dates. Your own offers remain available in My shifts & offers.'}</p></CardContent></Card>:
          dashboard.isLoading?<p>Loading team roster…</p>:dashboard.error?<div role="alert"><p>Unable to load this team. Your access may have expired or been revoked.</p><Button variant="outline" onClick={()=>void refresh()}>Refresh access</Button></div>:dashboard.data&&<>
            <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">{team?.siteName} · {zone}</p><div className="flex flex-wrap gap-2">{home.data?.isAdmin&&<><Button variant="outline" onClick={()=>open('skills')}>Skills catalogue</Button><Button variant="outline" onClick={()=>open('member')}>Add team member</Button><Button variant="outline" onClick={()=>open('grant')}><ShieldCheck className="mr-2 h-4 w-4"/>Grant lead access</Button></>}{dashboard.data.canSchedule&&<Button onClick={()=>open('shift')}>Create shift</Button>}</div></div>
            ...
            {dashboard.data.canSchedule&&<Coverage key={`${teamId}-${date}`} teamId={teamId} range={queryWindow} zone={zone}/>}
            {!shifts.length?<Card><CardContent className="py-8"><CalendarDays className="mb-3 h-7 w-7 text-primary"/><h2 className="font-semibold">No shifts in this view</h2><p className="mt-1 text-sm text-muted-foreground">Create a shift or choose another date. Leads see shifts covered by their access dates.</p></CardContent></Card>:shifts.map(shift=><ShiftCard key={`${shift.id}-${shift.version}`} shift={shift} zone={zone} skills={dashboard.data!.skills} busy={mutation.isPending} cancel={(id,name)=>{setCancel({id,name});setReason('');}}/>)}
            <Card><CardHeader><CardTitle>Team members in this view</CardTitle></CardHeader><CardContent><div className="divide-y">{dashboard.data.members.map(member=><div key={member.id} className="flex flex-wrap justify-between gap-2 py-3"><div className="font-medium">{member.name} <span className="ml-2 text-xs font-normal text-muted-foreground">{member.type}</span></div><p className="text-sm text-muted-foreground">{time(member.startAt,zone)} → {time(member.endAt,zone)}</p>{home.data?.isAdmin&&<Button variant="outline" size="sm" onClick={()=>setQualificationEmployee({id:member.employeeId,name:member.name})}>Qualifications · {member.name}</Button>}</div>)}</div>{!dashboard.data.members.length&&<p className="text-muted-foreground">HR must add dated membership before shifts can be offered.</p>}</CardContent></Card>
      ...
      <TabsContent value="mine" className="space-y-4 pt-3"><Availability/><p className="text-sm text-muted-foreground">Review your role, site, hours and planned break before accepting. Contact your lead to withdraw an accepted assignment.</p>
        {mine.isLoading?<p>Loading your shifts…</p>:mine.error?<p role="alert">Unable to load your assignments.</p>:!mine.data?.length?<Card><CardContent className="py-8"><h2 className="font-semibold">No assignments in this view</h2><p className="mt-2 text-muted-foreground">Offers appear here when your user account is linked to an employee record and a lead offers you a shift.</p></CardContent></Card>:mine.data.map(a=><Card key={a.id}><CardContent className="flex flex-wrap items-center justify-between gap-4 pt-6"><div><div className="flex items-center gap-3"><h2 className="text-lg font-semibold">{a.role}</h2><Badge variant={a.status==='accepted'?'default':'secondary'}>{a.status==='offered'&&new Date(a.startAt)<=new Date()?'Offer expired':a.status}</Badge></div><p className="mt-2">{a.teamName} · {kindLabels[a.kind]} · {a.siteName}{a.station?` · ${a.station}`:''}</p><p className="mt-1 text-sm">{time(a.startAt,a.timezone)} → {time(a.endAt,a.timezone)}</p><p className="mt-1 text-sm text-muted-foreground">{a.timezone} · {a.breakMinutes} min planned break</p>{a.requiredSkills?.length>0&&<p className="mt-2 text-sm">Required skills: {a.requiredSkills.map(skill=>skill.name).join(', ')}</p>}{a.cancellationReason&&<p className="mt-2 text-sm">Cancellation: {a.cancellationReason}</p>}</div>
    ...
    <Dialog open={!!modal} onOpenChange={value=>{if(!value&&!mutation.isPending)setModal(null);}}><DialogContent className="max-h-[90vh] overflow-auto"><DialogHeader><DialogTitle>{({site:'Add site',team:'Create team',member:'Add team member',grant:'Grant lead access',shift:'Create shift',skills:'Skills catalogue'})[modal||'site']}</DialogTitle><DialogDescription>{modal==='skills'?'Define the skills HR can record for employees and schedulers can require on shifts.':modal==='site'?'Choose the location and time zone used for its schedules.':modal==='team'?'Employment type stays on each employee profile. Choose the kind of work this team handles.':`All times below use ${zone}. Access and membership end at the exact end time.`}</DialogDescription></DialogHeader>
      <form className="space-y-4" onSubmit={save}>
        ...
        {modal==='shift'&&<div className="grid grid-cols-2 gap-3"><Field label="Occurrences (1 = single shift)"><Input required type="number" min={1} max={12} value={form.count} onChange={e=>set('count',e.target.value)}/></Field><Field label="Repeat every (days)"><Input required type="number" min={1} max={28} value={form.intervalDays} onChange={e=>set('intervalDays',e.target.value)}/></Field><p className="col-span-2 text-sm text-muted-foreground">Repeats preserve site local times, within 90 days. All occurrences must fit your access dates.</p></div>}
        ...
        {(modal==='shift'||modal==='skills')&&<fieldset className="space-y-2"><legend className="text-sm font-medium">{modal==='shift'?'Required skills (optional)':'Available skills'}</legend>{catalogue.isLoading?<p>Loading skill catalogue…</p>:catalogue.error?<div role="alert"><p>Unable to load skills.</p><Button type="button" variant="outline" onClick={()=>void catalogue.refetch()}>Retry skill catalogue</Button></div>:<SkillCatalogue skills={catalogue.data||[]} selected={modal==='shift'?requiredSkills:undefined} onChange={setRequiredSkills} canCreate={home.data?.isAdmin} disabled={mutation.isPending}/>}</fieldset>}
        <div className="flex justify-end gap-2"><Button variant="outline" type="button" disabled={mutation.isPending} onClick={()=>setModal(null)}>Close</Button><Button type="submit" className={modal==='skills'?'hidden':undefined} disabled={modal==='skills'||(modal==='shift'&&!catalogue.isSuccess)||mutation.isPending||((modal==='member'||modal==='grant')&&!form.personId)}>{mutation.isPending?'Saving…':'Save'}</Button></div>
      </form></DialogContent></Dialog>
    {qualificationEmployee&&<QualificationDialog key={qualificationEmployee.id} employee={qualificationEmployee} zone={zone} onClose={()=>setQualificationEmployee(null)}/>}
...
function ShiftCard({shift,zone,skills,busy,cancel}:{shift:ShiftView;zone:string;skills:WorkforceSkill[];busy:boolean;cancel:(id:number,name:string)=>void}) {
  ...
  const future=!shift.cancelledAt&&new Date(shift.startAt)>new Date();
  return <Card id={`shift-${shift.id}`} className="scroll-mt-4"><CardContent className="pt-6">{shift.cancelledAt&&<p className="mb-3 font-medium text-muted-foreground">Superseded shift · Retained as history</p>}<div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-lg font-semibold">{shift.role}{shift.station&&<span className="font-normal text-muted-foreground"> · {shift.station}</span>}</h2><p className="mt-1 text-sm">{time(shift.startAt,zone)} → {time(shift.endAt,zone)}</p><p className="mt-1 text-sm text-muted-foreground">{shift.breakMinutes} min planned break</p>{shift.requiredSkills.length>0&&<p className="mt-2 text-sm">Required skills: {shift.requiredSkills.map(id=>skills.find(s=>s.id===id)?.name||`Unavailable skill #${id}`).join(', ')}</p>}</div><Badge className="h-fit" variant={accepted>=shift.headcount?'default':'secondary'}>{accepted} / {shift.headcount} accepted</Badge></div>
    ...
    {shift.canSchedule&&<ShiftOperations shift={shift} zone={zone}/>}
  </CardContent></Card>;
}
