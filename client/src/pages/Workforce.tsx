...
import {RecurringShiftsButton, ShiftActions} from '@/components/hr/WorkforceRosters';
import {QualificationPicker,WorkforceStaffing,StaffingCandidatesButton} from '@/components/hr/WorkforceStaffing';
import {MembershipEndButton,ArrivalRulesPanel,MyArrival,ShiftOperationsButton} from '@/components/hr/WorkforceOperations';
import {QualificationRenewals} from '@/components/hr/WorkforceRenewals';
import {SkillCatalogue,QualificationDialog} from '@/components/workforce/Qualifications';
import {useToast} from '@/hooks/use-toast';
import {kindLabels, workforceKinds, siteTimeToIso, type WorkforceHome, type WorkforceDashboard, type WorkforcePerson, type MyAssignment, type ShiftView, type WorkforceSkill} from '@shared/workforce';
...
export default function Workforce() {
  ...
  const [teamId,setTeamId]=useState(()=>new URLSearchParams(window.location.search).get('teamId')||''),[modal,setModal]=useState<Modal>(null),[form,setForm]=useState<Record<string,string>>({});
  const [date,setDate]=useState(new Date().toISOString().slice(0,10));
  const [cancel,setCancel]=useState<{id:number;name:string}|null>(null),[reason,setReason]=useState('');
  const [requiredSkills,setRequiredSkills]=useState<number[]>([]),[skillsOpen,setSkillsOpen]=useState(false),[qualificationEmployee,setQualificationEmployee]=useState<{id:number;name:string}|null>(null);
  const catalogue=useQuery<WorkforceSkill[]>({queryKey:['/api/workforce/skills'],enabled:modal==='shift'||skillsOpen});
  ...
  async function save(event:FormEvent) {
    ...
    try {
      if(modal==='site') {url+='sites';body={name:form.name,timezone:form.timezone};}
        ...
        else {url+=`teams/${teamId}/shifts`;body={...dates,role:form.role,station:form.station||'',headcount:Number(form.headcount),breakMinutes:Number(form.breakMinutes),qualificationIds:JSON.parse(form.qualificationIds||'[]'),requiredSkills};}
      }
  ...
  const futureShifts=shifts.filter(s=>s.status==='scheduled'&&new Date(s.endAt)>new Date());
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue={home.data?.teams.length || home.data?.isAdmin?'team':'mine'}>
      <TabsList className="flex h-auto flex-wrap justify-start"><TabsTrigger value="team">Team workspace</TabsTrigger><TabsTrigger value="mine">My shifts & offers</TabsTrigger><TabsTrigger value="staffing">Availability & qualifications</TabsTrigger><TabsTrigger value="renewals">Qualification renewals</TabsTrigger></TabsList>
      <TabsContent value="team" className="space-y-5 pt-3">
        ...
        {!teamId?<Card><CardContent className="py-10"><Users className="mb-3 h-8 w-8 text-primary"/><h2 className="text-lg font-semibold">{home.data?.isAdmin?'Set up your first operational team':'No team-lead access assigned'}</h2><p className="mt-2 text-muted-foreground">{home.data?.isAdmin?'Add a site and its time zone, create a team, then add employees and dated lead access.':'HR can grant you access to a team for specific dates. Your own offers remain available in My shifts & offers.'}</p></CardContent></Card>:
          dashboard.isLoading?<p>Loading team roster…</p>:dashboard.error?<div role="alert"><p>Unable to load this team. Your access may have expired or been revoked.</p><Button variant="outline" onClick={()=>void refresh()}>Refresh access</Button></div>:dashboard.data&&<>
            <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">{team?.siteName} · {zone}</p><div className="flex flex-wrap gap-2">{home.data?.isAdmin&&<><Button variant="outline" onClick={()=>setSkillsOpen(true)}>Skills catalogue</Button><Button variant="outline" onClick={()=>open('member')}>Add team member</Button><Button variant="outline" onClick={()=>open('grant')}><ShieldCheck className="mr-2 h-4 w-4"/>Grant lead access</Button></>}{dashboard.data.canSchedule&&<><RecurringShiftsButton key={teamId} teamId={Number(teamId)} zone={zone}/><Button onClick={()=>{setRequiredSkills([]);open('shift');}}>Create shift</Button></>}</div></div>
            ...
            {!shifts.length?<Card><CardContent className="py-8"><CalendarDays className="mb-3 h-7 w-7 text-primary"/><h2 className="font-semibold">No shifts in this view</h2><p className="mt-1 text-sm text-muted-foreground">Create a shift or choose another date. Leads see shifts covered by their access dates.</p></CardContent></Card>:shifts.map(shift=><ShiftCard key={shift.id} shift={shift} zone={zone} skills={dashboard.data!.skills} busy={mutation.isPending}
              cancel={(id,name)=>{setCancel({id,name});setReason('');}}/>)}
            <Card><CardHeader><CardTitle>Team members in this view</CardTitle></CardHeader><CardContent><div className="divide-y">{dashboard.data.members.map(member=><div key={member.id} className="flex flex-wrap justify-between gap-2 py-3"><div className="font-medium">{member.name} <span className="ml-2 text-xs font-normal text-muted-foreground">{member.type}</span></div><p className="text-sm text-muted-foreground">{time(member.startAt,zone)} → {time(member.endAt,zone)}</p>{home.data?.isAdmin&&<><MembershipEndButton member={member} zone={zone}/><Button variant="outline" size="sm" onClick={()=>setQualificationEmployee({id:member.employeeId,name:member.name})}>Recorded skills</Button></>}</div>)}</div>{!dashboard.data.members.length&&<p className="text-muted-foreground">HR must add dated membership before shifts can be offered.</p>}</CardContent></Card>
            {home.data?.isAdmin&&<ArrivalRulesPanel key={teamId} teamId={Number(teamId)} zone={zone} members={dashboard.data.members}/>}
            {home.data?.isAdmin&&<Card><CardHeader><CardTitle>Lead access</CardTitle></CardHeader><CardContent className="space-y-3">{dashboard.data.grants.map(grant=><div key={grant.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3"><div><p className="font-medium">{grant.name} · {grant.permission==='schedule'?'Schedule & manage offers':grant.permission==='review_time'?'Review timesheets':grant.permission==='review_performance'?'Review performance':'View roster'}</p><p className="text-sm text-muted-foreground">{time(grant.startAt,zone)} → {time(grant.endAt,zone)}</p></div>{grant.revokedAt?<Badge variant="secondary">Revoked</Badge>:<Button variant="outline" disabled={mutation.isPending} onClick={()=>mutation.mutate({url:`/api/workforce/teams/${teamId}/grants/${grant.id}/revoke`,body:{}})}>Revoke access</Button>}</div>)}{!dashboard.data.grants.length&&<p className="text-muted-foreground">No lead access for these dates. Grant view, scheduling, timesheet or performance review access to an approved account.</p>}</CardContent></Card>}
          </>}
      </TabsContent>
      <TabsContent value="mine" className="space-y-4 pt-3"><p className="text-sm text-muted-foreground">Review your role, site, hours and planned break before accepting. Contact your lead to withdraw an accepted assignment.</p>
        {mine.isLoading?<p>Loading your shifts…</p>:mine.error?<p role="alert">Unable to load your assignments.</p>:!mine.data?.length?<Card><CardContent className="py-8"><h2 className="font-semibold">No assignments in this view</h2><p className="mt-2 text-muted-foreground">Offers appear here when your user account is linked to an employee record and a lead offers you a shift.</p></CardContent></Card>:mine.data.map(a=><Card key={a.id}><CardContent className="flex flex-wrap items-center justify-between gap-4 pt-6"><div><div className="flex items-center gap-3"><h2 className="text-lg font-semibold">{a.role}</h2><Badge variant={a.status==='accepted'?'default':'secondary'}>{a.status==='offered'&&new Date(a.startAt)<=new Date()?'Offer expired':a.status}</Badge></div><p className="mt-2">{a.teamName} · {kindLabels[a.kind]} · {a.siteName}{a.station?` · ${a.station}`:''}</p><p className="mt-1 text-sm">{time(a.startAt,a.timezone)} → {time(a.endAt,a.timezone)}</p><p className="mt-1 text-sm text-muted-foreground">{a.timezone} · {a.breakMinutes} min planned break</p>{a.replacesId&&a.status!=='cancelled'&&<p className="mt-2 text-sm font-medium">Revised shift: {a.changeReason}{a.status==='offered'&&'. Review these details before responding.'}</p>}{a.requiredSkills.length>0&&<p className="mt-2 text-sm">Required skills: {a.requiredSkills.map(s=>s.name).join(', ')}</p>}{a.requiredQualifications.length>0&&<p className="mt-2 text-sm">Required: {a.requiredQualifications.map(q=>q.name).join(', ')}</p>}{a.missingQualifications.length>0&&<p role="alert" className="mt-2 text-sm text-destructive">Qualification review needed: {a.missingQualifications.join(', ')}</p>}{a.replacementPending&&<p className="mt-2 text-sm">Replacement requested. You remain assigned until a replacement accepts or your lead cancels your assignment.</p>}{a.replacesAssignmentId&&a.status==='offered'&&<p className="mt-2 text-sm">Replacement offer: {a.replacementReason}</p>}{a.cancellationReason&&<p className="mt-2 text-sm">Cancellation: {a.cancellationReason}</p>}</div>
          {a.status==='offered'&&new Date(a.startAt)>new Date()&&<div className="flex gap-2"><Button variant="outline" disabled={mutation.isPending} onClick={()=>mutation.mutate({url:`/api/workforce/assignments/${a.id}/respond`,body:{decision:'declined'}})}>Decline</Button><Button disabled={mutation.isPending} onClick={()=>mutation.mutate({url:`/api/workforce/assignments/${a.id}/respond`,body:{decision:'accepted'}})}>Accept shift</Button></div>}{a.status==='accepted'&&<><MyArrival assignmentId={a.id} zone={a.timezone}/><ShiftOperationsButton shiftId={a.shiftId} zone={a.timezone} startAt={a.startAt} endAt={a.endAt}/></>}</CardContent></Card>)}
      </TabsContent>
      <TabsContent value="staffing" className="pt-4"><WorkforceStaffing isAdmin={!!home.data?.isAdmin}/></TabsContent>
      <TabsContent value="renewals" className="pt-4"><QualificationRenewals isAdmin={!!home.data?.isAdmin}/></TabsContent>
    </Tabs>
    ...
    <Dialog open={!!modal} onOpenChange={value=>{if(!value&&!mutation.isPending)setModal(null);}}><DialogContent className="max-h-[90vh] overflow-auto"><DialogHeader><DialogTitle>{({site:'Add site',team:'Create team',member:'Add team member',grant:'Grant lead access',shift:'Create shift'})[modal||'site']}</DialogTitle><DialogDescription>{modal==='site'?'Choose the location and time zone used for its schedules.':modal==='team'?'Employment type stays on each employee profile. Choose the kind of work this team handles.':`All times below use ${zone}. Access and membership end at the exact end time.`}</DialogDescription></DialogHeader>
      <form className="space-y-4" onSubmit={save}>
        ...
        {modal==='grant'&&<Field label="Permission"><select className={selectClass} value={form.permission} onChange={e=>set('permission',e.target.value)}><option value="view">View roster</option><option value="schedule">Create shifts and manage offers</option><option value="review_time">Review timesheets</option><option value="review_performance">Review performance</option></select></Field>}
        ...
        {modal==='shift'&&<QualificationPicker value={JSON.parse(form.qualificationIds||'[]')} onChange={ids=>set('qualificationIds',JSON.stringify(ids))}/>}
        {modal==='shift'&&<fieldset><legend className="text-sm font-medium">Required skills</legend>{catalogue.error?<p role="alert">Unable to load skill catalogue.</p>:<SkillCatalogue skills={catalogue.data||[]} selected={requiredSkills} onChange={setRequiredSkills} canCreate={false} disabled={mutation.isPending}/>}</fieldset>}
    ...
    <Dialog open={skillsOpen} onOpenChange={setSkillsOpen}><DialogContent><DialogHeader><DialogTitle>Skills catalogue</DialogTitle><DialogDescription>Maintain the skills required by workforce assignments.</DialogDescription></DialogHeader><SkillCatalogue skills={catalogue.data||[]} canCreate={home.data?.isAdmin}/></DialogContent></Dialog>
    {qualificationEmployee&&<QualificationDialog employee={qualificationEmployee} zone={zone} onClose={()=>setQualificationEmployee(null)}/>}
...
function ShiftCard({shift,zone,skills,busy,cancel}:{shift:ShiftView;zone:string;skills:WorkforceSkill[];busy:boolean;cancel:(id:number,name:string)=>void}) {
  const accepted=shift.assignments.filter(a=>a.status==='accepted').length;
  const future=shift.status==='scheduled'&&new Date(shift.startAt)>new Date();
  return <Card><CardContent className="pt-6"><div className="flex flex-wrap justify-between gap-3"><div><div className="mb-1 flex gap-2"><Badge variant="outline">#{shift.id} · {shift.status}</Badge>{shift.seriesId&&<Badge variant="outline">Recurring</Badge>}</div><h2 className="text-lg font-semibold">{shift.role}{shift.station&&<span className="font-normal text-muted-foreground"> · {shift.station}</span>}</h2><p className="mt-1 text-sm">{time(shift.startAt,zone)} → {time(shift.endAt,zone)}</p><p className="mt-1 text-sm text-muted-foreground">{shift.breakMinutes} min planned break</p></div><Badge className="h-fit" variant={accepted>=shift.headcount?'default':'secondary'}>{accepted} / {shift.headcount} accepted</Badge></div>
    {shift.changeReason&&<p className="mt-3 text-sm">{shift.status==='cancelled'?'Cancelled':shift.replacementId?`Replaced by shift #${shift.replacementId}`:shift.replacesId?`Revision of shift #${shift.replacesId}`:"Change"}: {shift.changeReason}</p>}
    {shift.requiredSkills.length>0&&<p className="mt-2 text-sm">Required skills: {shift.requiredSkills.map(id=>skills.find(s=>s.id===id)?.name||`Skill #${id}`).join(', ')}</p>}
    {shift.requiredQualifications.length>0&&<p className="mt-2 text-sm">Required: {shift.requiredQualifications.map(q=>q.name).join(', ')}</p>}
    <div className="mt-3 flex flex-wrap items-center gap-2"><ShiftActions shift={shift} zone={zone}/>{shift.canSchedule&&<ShiftOperationsButton shiftId={shift.id} zone={zone} startAt={shift.startAt} endAt={shift.endAt}/>}</div>
    <div className="mt-4 divide-y">{shift.assignments.map(a=><div key={a.id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><span className="font-medium">{a.name}</span><Badge variant="outline" className="ml-3">{a.status==='offered'&&!future?'Offer expired':a.status}</Badge>{a.missingQualifications.length>0&&<p className="mt-1 text-sm text-destructive">Qualification review needed: {a.missingQualifications.join(', ')}</p>}{a.replacementPending&&<p className="text-sm">Replacement pending</p>}{a.replacesAssignmentId&&<p className="text-sm">Replacement for assignment #{a.replacesAssignmentId}</p>}{a.cancellationReason&&<p className="mt-1 text-sm text-muted-foreground">{a.cancellationReason}</p>}</div>{shift.canSchedule&&future&&!a.replacementPending&&!shift.assignments.some(r=>r.replacesAssignmentId===a.id&&r.status==='accepted')&&(['offered','accepted'].includes(a.status)||accepted<shift.headcount)&&!(a.replacesAssignmentId&&a.status==='offered')&&<StaffingCandidatesButton shift={shift} source={a}/>} {shift.canSchedule&&future&&['offered','accepted'].includes(a.status)&&<Button size="sm" variant="ghost" disabled={busy} onClick={()=>cancel(a.id,a.name)}>Cancel assignment</Button>}</div>)}</div>
    {shift.canSchedule&&future&&accepted<shift.headcount&&<div className="mt-4 border-t pt-4"><StaffingCandidatesButton shift={shift}/></div>}
  </CardContent></Card>;
}
