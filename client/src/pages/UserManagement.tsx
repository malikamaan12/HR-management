...
export default function UserManagement() {
 ...
 async function applyAction() {
  ...
  try {
   ...
   if(action==='save') await mutation.mutateAsync({url:`/api/admin/users/${selected.id}`,method:'PATCH',body:{...base,username:selected.username,email:selected.email,role:selected.role,department:selected.department??''}});
    ...
    const result=await mutation.mutateAsync({url:`/api/admin/users/${selected.id}/${action}`,body:base});
   ...
   cache.invalidateQueries({queryKey:[`/api/admin/users/${selected.id}/history`]});setSelected(null);setConfirmed(false);notify();
 ...
 return <div className="space-y-6"><Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>Account management</CardTitle><Button variant="outline" onClick={()=>setShowCreate(!showCreate)}>Add user</Button></div><p className="text-sm text-muted-foreground">Manage sign-in access separately from employee records. Changes are audited and sign the affected user out.</p></CardHeader><CardContent className="space-y-6">
 ...
 {selected&&<Card id="account-details"><CardHeader><CardTitle>Manage {selected.firstName} {selected.lastName}</CardTitle><p className="text-sm text-muted-foreground">{selected.username} · {labels[selected.accountState]}</p></CardHeader><CardContent className="space-y-4">
  ...
  </div><label className="block">Action<select className="block w-full border rounded p-2" value={action} onChange={e=>{setAction(e.target.value);setConfirmed(false);}}><option value="save">Save username, email, role and department</option><option value="frozen">Freeze account</option><option value="on_hold">Hold access</option><option value="revoked">Revoke access</option><option value="active">Restore access</option><option value="password-setup">Require password reset and generate setup link</option><option value="revoke-sessions">Sign out all sessions</option><option value="unlock">Clear failed-login lockout</option>{selected.approvalStatus!=='approved'&&<option value="approve">Approve account</option>}<option value="deleted">Delete account access permanently</option></select></label>
