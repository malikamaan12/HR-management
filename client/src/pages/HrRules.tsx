...
export default function HrRules() {
 ...
 {kind === 'leave' && <><div className="flex gap-5">{['paid', 'balanceRequired'].map(k => <label key={k}><input type="checkbox" checked={config[k]} onChange={e => change(k, e.target.checked)}/> {k === 'paid' ? 'Paid leave' : 'Require available balance'}</label>)}</div><div className="grid gap-4 md:grid-cols-3"><Field label="Accrual"><select className={fieldClass} value={config.accrualMode} onChange={e => change('accrualMode', e.target.value)}><option value="none">Manual credits only</option><option value="annual">Annual grant on first eligible day</option><option value="monthly">Monthly after each full eligible month</option></select></Field>{numeric('annualDays', 'Annual grant (days)', 0.01)}{numeric('monthlyDays', 'Monthly accrual (days)', 0.01)}{numeric('carryoverLimit', 'Maximum carryover (days)', 0.01)}{numeric('minServiceDays', 'Minimum service (days)')}{numeric('maxConsecutiveDays', 'Maximum consecutive calendar days')}{approver}</div><label className="block"><input type="checkbox" checked={config.allowHalfDays||false} onChange={e=>change('allowHalfDays',e.target.checked)}/> Allow half-day leave on one office workday</label><Field label="Additional approval stages (in order)"><div className="space-y-2">{(config.additionalApproverIds||[]).map((id:number,i:number)=><div className="flex gap-2" key={i}><select className={fieldClass} required value={id||''} onChange={e=>change('additionalApproverIds',config.additionalApproverIds.map((v:number,j:number)=>j===i?Number(e.target.value):v))}><option value="">Choose approver for stage {i+2}</option>{directory.data?.approvers.map(p=><option key={p.id} value={p.id}>{p.firstName} {p.lastName} · {p.role}</option>)}</select><Button type="button" variant="outline" onClick={()=>change('additionalApproverIds',config.additionalApproverIds.filter((_:number,j:number)=>j!==i))}>Remove stage</Button></div>)}<Button type="button" variant="outline" disabled={(config.additionalApproverIds||[]).length>=3} onClick={()=>change('additionalApproverIds',[...(config.additionalApproverIds||[]),0])}>Add approval stage</Button></div></Field><p className="text-sm">Up to four independent stages. Leave stays pending and reserves balance until the final approval. Half-days count as 0.5 day; the office schedule defines each half.</p></>}
