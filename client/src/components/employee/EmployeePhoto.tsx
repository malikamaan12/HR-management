...
export default function EmployeePhoto({employee}: {employee:ApiEmployeeRecord}) {
 const {t,language}=useLocale();
  ...
  return <><button type="button" className="employee-photo-trigger" aria-label={`${t(employee.photo?'Preview photo for':'Add photo for')} ${employee.firstName} ${employee.lastName}`} disabled={!employee.photo&&!employee.access.canEdit} onClick={()=>setOpen(true)}><EmployeeAvatar employee={employee} className="h-24 w-24"/>{employee.access.canEdit&&<span className="employee-photo-camera"><Camera className="h-4 w-4"/></span>}</button><div className="employee-photo-caption">{employee.access.canEdit?<Button variant="ghost" size="sm" onClick={()=>setOpen(true)}><Camera className="me-2 h-4 w-4"/>{employee.photo?t('Change photo'):t('Upload photo')}</Button>:employee.photo&&<Button variant="ghost" size="sm" onClick={()=>setOpen(true)}>{t("Preview photo")}</Button>}</div><Dialog open={open} onOpenChange={setOpen}><DialogContent className="sm:max-w-md">{open&&<PhotoEditor key={`${employee.id}-${open}`} employee={employee} onClose={()=>setOpen(false)}/>}</DialogContent></Dialog></>;
}
...
function PhotoEditor({employee,onClose}:{employee:ApiEmployeeRecord;onClose:()=>void}) {
 const {t,language}=useLocale();
  ...
  },onSuccess:()=>{cache.invalidateQueries({predicate:query=>typeof query.queryKey[0]==='string'&&query.queryKey[0].startsWith('/api/employees')});toast({title:remove?t('Employee photo removed'):t('Employee photo saved')});onClose();}});
  ...
  return <><DialogHeader><DialogTitle>{employee.access.canEdit?t('Employee photo'):t('Photo preview')}</DialogTitle><DialogDescription>{employee.firstName} {employee.lastName} · {employee.employeeId}</DialogDescription></DialogHeader><div className="photo-preview-stage"><EmployeeAvatar employee={{...employee,photo:remove?null:preview||employee.photo}} className="h-48 w-48 rounded-3xl"/>{preparing&&<span role="status"><Loader2 className="h-5 w-5 animate-spin"/>{t("Preparing preview…")}</span>}</div>{employee.access.canEdit&&<><input ref={input} type="file" accept="image/jpeg,image/png,image/webp" aria-label={t("Choose employee photo")} className="sr-only" disabled={preparing||mutation.isPending} onChange={event=>{const value=event.target.files?.[0];event.target.value='';if(value)void choose(value);}}/><Button variant="outline" className="w-full" disabled={preparing||mutation.isPending} onClick={()=>input.current?.click()}><ImagePlus className="me-2 h-4 w-4"/>{file?t('Choose another photo'):t('Choose photo')}</Button><p className="text-center text-xs text-muted-foreground">{t("JPG, PNG or WebP · up to 10 MB")}<br/>{t("A square preview is saved for the employee directory.")}</p>{(error||mutation.error)&&<p role="alert" className="text-sm text-destructive">{t(error||mutation.error?.message||'')}</p>}{remove&&<p role="status" className="rounded-xl bg-muted p-3 text-sm">{t("The current photo will be removed when you save.")}</p>}<div className="flex flex-wrap items-center gap-2">{employee.photo&&<Button variant="ghost" disabled={preparing||mutation.isPending} onClick={()=>{setFile(null);setRemove(!remove);}}><Trash2 className="me-2 h-4 w-4"/>{remove?t('Keep photo'):t('Remove')}</Button>}<div className="ms-auto flex gap-2"><Button variant="outline" disabled={mutation.isPending} onClick={onClose}>{t("Cancel")}</Button><Button disabled={preparing||mutation.isPending||(!file&&!remove)} onClick={()=>mutation.mutate()}>{mutation.isPending?t('Saving…'):t('Save photo')}</Button></div></div></>}</>;
}
