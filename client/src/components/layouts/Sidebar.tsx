...
export default function Sidebar({className}:{className?:string}){
 ...
 <nav aria-label="Main navigation" className="flex-1 overflow-y-auto p-3">{[...new Set(filtered.map(p=>p.section))].map(section=><section key={section} className="mb-4"><h2 className="px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">{section}</h2><ul className="space-y-1">{filtered.filter(p=>p.section===section).map(p=><li key={p.href}><Link href={p.href} aria-current={pageForPath(location)?.href===p.href?'page':undefined} className={cn('flex items-center gap-3 rounded-md px-3 py-2 text-sm hover:bg-accent focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary',pageForPath(location)?.href===p.href&&'bg-primary/10 text-primary font-semibold')}><i aria-hidden="true" className={`fas fa-${p.icon} w-5 text-center`}/>{pageLabel(p,user!.role)}</Link></li>)}</ul></section>)}{!filtered.length&&<p role="status" className="p-3 text-sm text-muted-foreground">No accessible modules match your search.</p>}</nav>
