  ...
  return <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>Service readiness & recovery</CardTitle><Button variant="outline" disabled={status.isFetching || busy} onClick={() => void status.refetch()}>{status.isFetching ? 'Refreshing…' : 'Refresh service status'}</Button></div></CardHeader><CardContent className="space-y-5">
    ...
    {data && <>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {data.release&&<article className="rounded-lg border p-4 space-y-2"><h3 className="font-semibold">Release evidence</h3><p className="break-all text-sm">Reported revision: {data.release.revision||'Not supplied by deployment'}</p><p className="text-sm">Applied migrations: {data.release.appliedMigrations??'Unknown'} / expected {data.release.expectedMigrations??'Unknown'}.</p><p className="text-xs text-muted-foreground">Compare the reported revision with the reviewed release. Matching migration counts do not verify migration contents or recovery.</p></article>}
        {data.security&&<article className="rounded-lg border p-4 space-y-2"><h3 className="font-semibold">Account and upload protection</h3><p className="text-sm">Authenticator: {data.security.mfaConfigured?'configured':'encryption key required'}; privileged enrollment {data.security.mfaEnforced?'required':'not enforced'}.</p><p className="text-sm">Malware scanner: {data.security.scannerConfigured?'configured — availability checked on each upload':'not configured'}; scanning {data.security.scannerRequired?'required':'optional'}.</p></article>}
        {data.scheduler&&<article className="rounded-lg border p-4 space-y-2"><h3 className="font-semibold">Scheduled work</h3>{!data.scheduler.historyAvailable&&<p role="alert">Job history is unavailable. Check database connectivity and migrations.</p>}<p className="text-sm">{data.scheduler.mode==='external'?`External trigger: ${data.scheduler.externalConfigured?'credentials configured; verify invocations below':'credentials missing'}`:'In-process scheduling stops while the host is asleep. Provision an external scheduler for dependable delivery.'}</p><ul className="text-sm space-y-1">{data.scheduler.jobs.map(job=><li key={job.name}>{job.name}: {job.lastStatus||'never run'}; failures {job.failures}; last finished {job.lastFinishedAt?new Date(job.lastFinishedAt).toLocaleString():'never'}.</li>)}</ul></article>}
