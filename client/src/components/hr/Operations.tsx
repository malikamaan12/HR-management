...
export function Section({ title, children }: {
    title: string;
    children: ReactNode;
}) { return <section className="module-card space-y-4 rounded-2xl border bg-card p-4 sm:p-5"><h2 className="text-base font-semibold">{title}</h2>{children}</section>; }
export function Table({ headers, rows, pageSize, responsive=true }: {
 ...
 return <div className="space-y-3"><div className={`module-table-wrap overflow-auto rounded-xl border ${responsive?'module-responsive-table':''}`}><table className="module-table w-full text-sm"><thead><tr>{headers.map((h,i) => <th scope="col" className="bg-muted/60 p-3 text-left text-xs font-medium text-muted-foreground" key={i}>{h||<span className="sr-only">Actions</span>}</th>)}</tr></thead><tbody>{shown.map((row, i) => <tr className="border-t" key={current*size+i}>{row.map((cell, j) => <td data-label={headers[j]||'Actions'} className="p-3 align-top" key={j}><div>{typeof cell==='string'?<StatusPill value={cell}/>:cell}</div></td>)}</tr>)}</tbody></table>{!rows.length && <EmptyState title="No matching records" description="Records will appear here when available."/>}</div>{pageSize&&rows.length>pageSize&&<div className="flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground"><span>{current*size+1}–{Math.min((current+1)*size,rows.length)} of {rows.length}</span><div className="flex items-center gap-2"><Button type="button" size="sm" variant="outline" disabled={current===0} onClick={()=>setPage(current-1)}>Previous</Button><span>Page {current+1} of {pages}</span><Button type="button" size="sm" variant="outline" disabled={current+1>=pages} onClick={()=>setPage(current+1)}>Next</Button></div></div>}</div>;
}
