...
function PackageEditor({ data, onClose }: { data: PackageData; onClose: () => void }) {
  ...
  return <form className="space-y-5 rounded-lg border p-4" onSubmit={event => { event.preventDefault(); setError(''); const parsed = compensationRevisionInput.safeParse({ expectedVersion, effectiveFrom, definition, reason }); if (!parsed.success) { setError(parsed.error.issues.map(issue => `${issue.path.join('.')}: ${issue.message}`).join('; ')); return; } save.mutate(parsed.data); }}>
    ...
    <p className="text-sm text-muted-foreground">Saving preserves previous revisions. An administrator can separately publish monthly cash or hourly base pay to payroll after reviewing the package.</p>
...
function PayrollSyncForm({ data, revision, onClose }: { data: PackageData; revision: CompensationRevision; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ cycleStartDay: previous?.cycleStartDay || 1, payDay: previous?.payDay || 1, regularMinutesPerDay: previous?.regularMinutesPerDay || 480, overtimeMultiplier: previous?.overtimeMultiplier || 1, hourlyRate: previous?.hourlyRate || '0.00', approverId: previous?.approverId || 0, reason: '', confirmed: false });
  const [error, setError] = useState('');
  const base = revision.definition.items.find(item => item.category === 'base')!;
  const eligible = base.provision === 'cash' && ['monthly', 'hourly'].includes(base.frequency);
  ...
  return <form className="space-y-4 rounded-lg border p-4" onSubmit={event => { event.preventDefault(); setError(''); save.mutate(); }}><h3 className="font-semibold">Publish payroll rule from package v{revision.version}</h3>
    <p className="text-sm">Effective {revision.effectiveFrom}: {base.label} {revision.definition.currency} {base.amount} / {words(base.frequency)}. Monthly cash allowances: {allowances.map(item => `${item.label}: ${item.amount}`).join(', ') || 'none'}.</p>
    <p className="text-sm text-muted-foreground">This replaces the effective base pay and monthly allowance list for future payroll generation. Existing deduction rules are retained. Annual or one-time cash, reimbursements and provided benefits stay in the package. Saved payroll runs keep their recorded amounts; regenerate eligible drafts explicitly if needed.</p>
    {!eligible && <p role="alert">Only monthly or hourly cash base pay can map to payroll. Daily and per-event rates remain available in the package and workforce pay setup.</p>}
    ...
    <div className="grid gap-3 sm:grid-cols-2">{([{ key: 'cycleStartDay', label: 'Cycle start day', min: 1, max: 28, step: 1 }, { key: 'payDay', label: 'Payment day', min: 1, max: 28, step: 1 }, { key: 'regularMinutesPerDay', label: 'Regular minutes per day', min: 1, max: 1440, step: 1 }, { key: 'overtimeMultiplier', label: 'Overtime multiplier', min: 1, max: 5, step: 0.01 }] as const).map(field => <label key={field.key} className={labelClass}>{field.label}<Input required type="number" min={field.min} max={field.max} step={field.step} value={form[field.key]} onChange={e => setForm({ ...form, [field.key]: Number(e.target.value) })} /></label>)}
      {base.frequency === 'monthly' && <label className={labelClass}>Overtime hourly rate ({revision.definition.currency})<Input required type="number" min="0" max="999999999.99" step="0.01" value={form.hourlyRate} onChange={e => setForm({ ...form, hourlyRate: e.target.value })}/></label>}
    ...
    </div><label className={labelClass}>Reason<Textarea required minLength={5} maxLength={2000} value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })} /></label>
    <label className="flex gap-2 text-sm"><input type="checkbox" required checked={form.confirmed} onChange={e => setForm({ ...form, confirmed: e.target.checked })} />I reviewed the replacement base pay, allowance list, deductions and effective date.</label>
