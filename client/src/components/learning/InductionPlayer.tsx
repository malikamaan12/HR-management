...
import {inductionStatus} from './InductionReports';
...
type EnrollmentRow={id:number;version:number;status:string;progress:number;score:number|null;dueDate:string|null;expiresOn:string|null;completedAt:string|null;certificateNumber:string|null;completionNote:string|null;approverId:number;courseSnapshot:{title:string;description?:string};history:WorkflowEvent[]};
...
export default function InductionPlayer({id,onBack}:{id:number;onBack?:()=>void}){
  ...
  const firstLesson=query.data?.content.lessons[0]?.id;
  ...
  if(!query.data)return <div className="space-y-4">{onBack&&<Button variant="outline" onClick={onBack}>Back to academy</Button>}<QueryError error={query.error}/>{query.isLoading?<p role="status">Loading training…</p>:<Button variant="outline" onClick={()=>query.refetch()}>Reload training</Button>}</div>;
  ...
  return <div className="space-y-6">
    {onBack&&<Button variant="outline" onClick={()=>{if(!detail.activeAttempt||!detail.canLearn||window.confirm('Leave this quiz? Unsubmitted answers will be cleared and the attempt timer will continue.'))onBack();}}>Back to academy</Button>}
    <header className="rounded-lg border border-t-4 bg-card p-5 space-y-3" style={{borderTopColor:detail.brand.accentColor}}>
      <p className="text-sm font-medium text-muted-foreground">{detail.brand.organizationName} · {detail.brand.academyTitle}</p><h2 className="text-2xl font-semibold">{row.courseSnapshot.title}</h2>
      <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm"><p>{detail.employeeName}</p><p>Release {detail.releaseNumber}</p><p>{detail.required?'Required training':'Optional training'}</p><p className="font-semibold">{inductionStatus(row.status)}</p></div>
      <div className="grid gap-2 text-sm sm:grid-cols-3"><p>Required lessons: {completedRequired} / {requiredLessons.length}</p><p>Due: {row.dueDate||'No deadline'}</p><p>Recorded quiz score: {row.score===null?'No score yet':`${row.score}%`}</p></div>
      <label className="block text-sm">Lesson progress: {row.progress}%<progress className="mt-2 block h-3 w-full" max={100} value={row.progress} aria-label="Training lesson progress"/></label>
    ...
    <div className="grid gap-5 lg:grid-cols-[17rem_minmax(0,1fr)]">
      <Section title="Course lessons"><ol className="space-y-2">{content.lessons.map((item,index)=>{
        ...
        return <li key={item.id}><button type="button" className={`w-full rounded-md border p-3 text-left text-sm ${lesson?.id===item.id?'border-primary bg-muted':''}`} onClick={()=>setSelectedLessonId(item.id)} aria-current={lesson?.id===item.id?'step':undefined}><span className="block font-medium">{index+1}. {item.title}</span><span className="mt-1 block text-xs text-muted-foreground">{progress?.completedAt?'Completed':blocked&&canStudy?'Complete earlier required lessons':progress?.openedAt?'Started':'Not started'} · {item.required?'Required':'Optional'}{item.estimatedMinutes?` · ${item.estimatedMinutes} min`:''}</span></button></li>;
      })}</ol><p className="text-xs text-muted-foreground">{settings.sequentialLessons?'Required lessons follow the order shown.':'Lessons may be completed in any order.'}</p></Section>
      {lesson&&<Section title={lesson.title}>
        {locked&&canStudy&&!selectedProgress?.completedAt?<p className="rounded bg-muted p-4 text-sm">Complete the earlier required lessons to begin this lesson.</p>:<>
          {!lessonVisible&&<div className="space-y-3"><p className="text-sm text-muted-foreground">Open this lesson to begin reviewing its content.</p><Button disabled={mutation.isPending} onClick={()=>mutation.mutate({url:`${base}/enrollments/${id}/lessons/${lesson.id}/open`,body:{},kind:'open'})}>Open lesson</Button></div>}
          {lessonVisible&&<div className="space-y-4">
            {lesson.body&&<div className="whitespace-pre-wrap break-words leading-7 text-sm">{lesson.body}</div>}
    ...
    <Section title="Knowledge check">
      ...
      <details className="rounded border p-4" open={detail.attempts.length>0&&!detail.activeAttempt}><summary className="cursor-pointer text-sm font-medium">Quiz attempt history ({detail.attempts.length})</summary><div className="mt-3"><Table headers={['Attempt','Started','Submitted','Score','Result']} rows={detail.attempts.map(a=>[a.attemptNumber,when(a.startedAt),when(a.submittedAt),a.score===null?'—':`${a.score}%`,a.passed?'Passed':a.submittedAt?'Not passed':inductionStatus(a.status)])}/></div></details>
    </Section>
...
function QuizAttempt({attempt,pending,now,onRefresh,onSubmit}:{attempt:ActiveAttempt;pending:boolean;now:number;onRefresh:()=>void;onSubmit:(answers:{questionId:string;optionIds:string[]}[])=>void}){
  const [answers,setAnswers]=useState<Record<string,string[]>>({}),[ready,setReady]=useState(false);
  ...
  return <form className="space-y-5" onSubmit={e=>{e.preventDefault();onSubmit(attempt.questions.filter(q=>answers[q.id]?.length).map(q=>({questionId:q.id,optionIds:answers[q.id]})));}}>
