...
export function StatusPill({value,label}:{value:string;label?:string}){const key=value.toLowerCase().replaceAll(' ','_');const tone=statusTones[key]||({processed:'positive',fulfilled:'positive',reimbursed:'positive',acknowledged:'positive',submitted:'attention',issued:'attention',return_requested:'attention',available:'positive',maintenance:'attention',lost:'negative',on_leave:'attention',expiring_soon:'attention',screening:'attention',shortlisted:'positive'} as Record<string,string>)[key];if(!tone)return <>{label??value}</>;return <span className={cn('status-pill',statusClasses[tone])}><span className="h-1.5 w-1.5 rounded-full bg-current" aria-hidden="true"/>{label??value.replaceAll('_',' ')}</span>;}
