...
export function StatusPill({value}:{value:string}){const tone=statusTones[value.toLowerCase()];if(!tone)return <>{value}</>;return <span className={cn('status-pill','status-'+tone)}><span className="h-1.5 w-1.5 rounded-full bg-current" aria-hidden="true"/>{value.replaceAll('_',' ')}</span>;}
export function MetricCard({label,value,detail,icon:Icon=BarChart3,tone='primary'}:{label:string;value:ReactNode;detail?:string;icon?:LucideIcon;tone?:'primary'|'positive'|'attention'}){return <div className={cn('metric-card','metric-'+tone)}><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium text-muted-foreground">{label}</p><Icon aria-hidden="true" className="h-5 w-5 shrink-0"/></div><p className="mt-2 text-2xl sm:text-3xl font-semibold tracking-tight tabular-nums">{value}</p>{detail&&<p className="mt-2 text-xs text-muted-foreground">{detail}</p>}</div>;}
