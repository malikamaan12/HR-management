...
export function CandidateCorrections({isOpen,onClose,candidateId}:{isOpen:boolean;onClose:()=>void;candidateId:number}){
 ...
 const record=useQuery<RecordData>({queryKey:[`/api/hiring/candidates/${candidateId}/correction-record`],enabled:isOpen});
 const history=useQuery<{items:any[];hasMore:boolean}>({queryKey:[`/api/hiring/candidates/${candidateId}/corrections`,{page}],enabled:isOpen});
...
function CorrectionForm({record}:{record:RecordData}){
 ...
 const save=useMutation({mutationFn:()=>{const fields=Object.fromEntries(Object.entries(values).filter(([key,value])=>value!==((record.candidate as any)[key]||'')).map(([key,value])=>[key,value||null]));return apiJson(`/api/hiring/candidates/${record.candidate.id}/corrections`,'POST',{expectedVersion:record.candidate.recordVersion,fields,reason});},onSuccess:async()=>{setSaved(true);await Promise.all([client.invalidateQueries({queryKey:['/api/hiring']}),client.invalidateQueries({queryKey:[`/api/hiring/candidates/${record.candidate.id}/correction-record`]}),client.invalidateQueries({queryKey:[`/api/hiring/candidates/${record.candidate.id}/corrections`]})]);}});
