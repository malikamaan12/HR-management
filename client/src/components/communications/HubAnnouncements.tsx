...
import { Megaphone, Plus, Search, RefreshCw, Users, Calendar, Pin, CheckCheck, ArrowUpRight, MailOpen, ClipboardCheck, LayoutGrid } from 'lucide-react';
...
export default function HubAnnouncements({ context, initialFilter = 'all' }: { context: HubContext; initialFilter?: string }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Announcements & briefings</h2><p className="mt-1 text-sm text-muted-foreground">One place for company notices, team instructions and required acknowledgements.</p></div>{context.canManage && <Button className="gap-2" onClick={() => setCreate(true)}><Plus className="h-4 w-4"/>New announcement</Button>}</div>
    <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{[
      { value: 'all', label: 'For you', Icon: Megaphone }, { value: 'unread', label: 'Unread notices', Icon: MailOpen },
      { value: 'needs_ack', label: 'Needs acknowledgement', Icon: ClipboardCheck }, { value: 'pinned', label: 'Pinned notices', Icon: Pin },
    ].map(item => <button key={item.value} className={`flex items-center gap-3 rounded-xl border bg-card p-4 text-left hover:bg-accent ${!managed && filter === item.value ? 'ring-1 ring-primary' : ''}`} onClick={() => { setManaged(false); changeFilter(item.value); }}><item.Icon className="h-5 w-5 shrink-0 text-primary"/><span><strong className="block text-xl tabular-nums">{counts?.mine[item.value as keyof BulletinOverview['mine']] ?? '—'}</strong><span className="text-xs text-muted-foreground">{item.label}</span></span></button>)}</div>
    ...
    <div className="flex items-center gap-3"><div className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground"/><input className={fieldClass + ' pl-9'} aria-label="Search announcements" placeholder="Find an announcement by title or wording…" maxLength={100} value={q} onChange={e => { setQ(e.target.value); setOffset(0); }}/></div><Button size="icon" variant="outline" aria-label="Refresh announcements" onClick={() => { void items.refetch(); void overview.refetch(); }}><RefreshCw className={`h-4 w-4 ${items.isFetching ? 'animate-spin' : ''}`}/></Button></div>
    ...
    <Pager offset={offset} hasMore={!items.error && items.data?.hasMore} onChange={setOffset}/>
  </div>;
}
