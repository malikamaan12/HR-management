...
export default function HubInbox({ onAnnouncements }: { onAnnouncements: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="space-y-3">{items.map(item => { const related = pages.find(page => item.url === page.href || item.url?.startsWith(page.href + '/')); return <article key={item.id} className={`flex items-start gap-3 rounded-xl border bg-card p-4 sm:gap-4 ${!item.read_at ? 'border-l-4 border-l-primary' : ''}`}>
