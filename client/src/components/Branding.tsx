...
export function BrandLogo({className='h-10 w-12',appearance}:{className?:string;appearance?:'light'|'dark'}){
  const {data=defaultPublicBranding}=useBranding(),{theme}=useTheme();
  const [failed,setFailed]=useState('');
  const src=(appearance||theme)==='dark'?data.assets.darkLogo||data.assets.lightLogo:data.assets.lightLogo||data.assets.darkLogo;
