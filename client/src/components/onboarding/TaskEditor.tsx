...
export type EditableTask={id:number;version:number;status:string;comments?:string|null;assigneeId?:number|null;assigneeName?:string|null;dueDate:string};
...
function TaskForm({task,close}:{task:EditableTask;close:()=>void}){
 ...
 return <form className="mt-3 min-w-64 space-y-2 rounded border p-3" onSubmit={e=>{e.preventDefault();save.mutate();}}><fieldset disabled={save.isPending} className="space-y-2">
  <label className="grid gap-1">Task status<select className="rounded border bg-background p-2" value={status} onChange={e=>setStatus(e.target.value)}><option value="not_started">Not started</option><option value="in_progress">In progress</option><option value="completed">Completed</option></select></label>
