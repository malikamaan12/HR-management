...
export async function casePolicy(tx:HelpdeskTransaction,requesterId:number,category:string,confidential:boolean,now:Date){
  ...
  return {assigneeId:handler?.id||null,status:handler?'in_progress' as const:'open' as const,
    ...
    firstResponseDueAt:new Date(now.getTime()+policy.firstResponseHours*3600000),resolutionDueAt:new Date(now.getTime()+policy.resolutionHours*3600000)};
}
