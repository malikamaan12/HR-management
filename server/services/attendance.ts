...
export async function clockAttendance(userId:number,action:'in'|'out'|'break_start'|'break_end',location?:string,notes?:string,now=new Date()){
  return db.transaction(async tx=>{
    ...
    if(action==='in'){
      ...
      const value={checkIn:now,status:await clockStatus(tx,employee,date,now,policy),checkInMethod:'mobile_app' as const,location:location || null,notes:notes || null};
