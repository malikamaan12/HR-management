...
import {assignmentReviews as reviews,workforceAssignments as assignments,workforceShifts as shifts,workforceTeams as teams,workforceTimesheets as timesheets,employees} from '@shared/schema';
...
router.get('/',reviewHandle(async(req,res)=>{
  ...
  const result=await db.transaction(async tx=>{
    ...
    return {team,...range,staffingThrough,staffing:{...staffingCounts,gaps},time,reviews:reviewSummary};
  });res.json(result);
}));
