...
import {separationManager,separationInputSchema,separationPolicySchema,separationStatuses,documentKinds,type SeparationSnapshot} from '@shared/separation';
...
router.put('/policy',handle(async(req,res)=>{
  manager(req);const input=z.object({version:z.number().int().nonnegative(),definition:separationPolicySchema,reason}).strict().parse(req.body);
  res.json(await db.transaction(async tx=>{
    await tx.execute(sql`LOCK TABLE separation_policies IN EXCLUSIVE MODE`);const previous=await separationPolicy(tx);checkVersion(previous,input.version);
    const row=(await tx.execute(sql`INSERT INTO separation_policies(version,definition,created_by,reason) VALUES(${previous.version+1},${JSON.stringify(input.definition)}::jsonb,${req.user.userId},${input.reason}) RETURNING *`)).rows[0];
    await recordHistory(tx,req,'separation_policy',row,input.reason);return {version:row.version};
  }));
}));
...
router.post('/:id/:action',handle(async(req,res)=>{
  ...
  res.json(await db.transaction(async tx=>{
    ...
    let status=row.status,snapshot=row.snapshot,reviewedBy=row.reviewed_by,reviewedAt=row.reviewed_at,preparedBy=row.prepared_by,completedBy=null,completedAt=null;
    if(action==='submit'){
      ...
      if(fingerprint(financial(current))!==fingerprint(financial(snapshot)))fail(409,'The approved calculation no longer matches current records. Cancel and prepare a new reviewed case.');
