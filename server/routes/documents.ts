...
router.post('/:id/renewal-requests',receiveDocument,async(req,res)=>{
  ...
  try{
    ...
    const result=await db.transaction(async tx=>{
      ...
      const [created]=await tx.insert(renewals).values({documentId:id,requestedBy:req.user!.userId,createdAt:new Date(),expectedVersion:input.expectedVersion,proposal:{...input,documentFile:key},policySnapshot:reviewPolicy,reviewDueDate:reviewDueDate(reviewPolicy.reviewDays)}).returning();
...
router.get('/renewal-requests/:requestId/history',async(req,res)=>{
 try{
  ...
  const [visible]=await db.select({id:renewals.id}).from(renewals).innerJoin(documents,eq(documents.id,renewals.documentId)).innerJoin(employees,eq(documents.employeeId,employees.id)).where(and(eq(renewals.id,id),renewalVisibility(req)));
  if(!visible)throw new DocumentError(404,'Renewal request not found');
  const r=await db.execute(sql`SELECT version,actor_id,reason,created_at,snapshot FROM hr_workflow_history WHERE kind='document_renewal' AND record_id=${id} ORDER BY version DESC LIMIT 21 OFFSET ${offset}`);
