...
import {employees, users, workforceSites as sites, workforceTeams as teams, workforceMembers as members,
  workforceGrants as grants, workforceShifts as shifts, workforceAssignments as assignments, skills, employeeSkills} from '@shared/schema';
...
router.post('/teams/:teamId/members',handle(async(req,res)=>{
  ...
  const result=await db.transaction(async tx=>{
    ...
    const [employee]=await tx.select({id:employees.id}).from(employees).where(and(eq(employees.id,input.employeeId),eq(employees.status,'active')));
    if(!employee) fail(400,'Choose an active employee');
...
router.get('/teams/:teamId/dashboard',handle(async(req,res)=>{
  ...
  const result=await db.transaction(async tx=>{
    ...
    const roster=visibleShifts.length?await tx.select({id:assignments.id,shiftId:assignments.shiftId,employeeId:employees.id,name:sql<string>`${employees.firstName} || ' ' || ${employees.lastName}`,status:assignments.status,cancellationReason:assignments.cancellationReason})
    ...
    const memberRows=await tx.select({id:members.id,employeeId:employees.id,name:sql<string>`${employees.firstName} || ' ' || ${employees.lastName}`,type:employees.type,startAt:members.startAt,endAt:members.endAt})
    ...
    return {team,from,to,canSchedule:isAdmin||permissions.some(g=>g.permission==='schedule'),members:memberRows,grants:grantRows,skills:requiredCatalogue,
      shifts:visibleShifts.map(s=>({...s,createdBy:undefined,canSchedule:isAdmin||permissions.some(g=>g.permission==='schedule' && g.startAt<=s.startAt && g.endAt>=s.endAt),assignments:roster.filter(a=>a.shiftId===s.id)}))};
  });res.json(result);
}));
router.post('/teams/:teamId/shifts',handle(async(req,res)=>{
  ...
  const result=await db.transaction(async tx=>{await teamAccess(tx,req.user!,teamId,'schedule',input);
    ...
    const [row]=await tx.insert(shifts).values({...input,teamId,createdBy:req.user!.userId}).returning();await audit(tx,req.user!,'shift',row.id,'Shift created');return row;});
  res.status(201).json(result);
}));
...
router.get('/my-assignments',handle(async(req,res)=>{
  ...
  const rows=await db.select({id:assignments.id,status:assignments.status,cancellationReason:assignments.cancellationReason,role:shifts.role,station:shifts.station,startAt:shifts.startAt,endAt:shifts.endAt,breakMinutes:shifts.breakMinutes,requiredSkills:shifts.requiredSkills,
  ...
  res.json(rows.map(row=>({...row,requiredSkills:row.requiredSkills.map(id=>catalogue.find(skill=>skill.id===id)??{id,name:`Unavailable skill #${id}`,category:null})})));
}));
router.post('/assignments/:id/respond',handle(async(req,res)=>{
  ...
  const result=await db.transaction(async tx=>{
    ...
    if(decision==='accepted') {
      ...
      if(await capacityCount(tx,shift.id)>=shift.headcount) fail(409,'This shift is fully staffed; contact your team lead');
    }
