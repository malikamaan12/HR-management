...
import {employees, users, workforceSites as sites, workforceTeams as teams, workforceMembers as members,
  workforceGrants as grants, workforceShifts as shifts, workforceAssignments as assignments} from '@shared/schema';
import {workforceAdmin, positiveId, siteInput, teamInput, memberInput, grantInput, shiftInput} from '@shared/workforce';
import {WorkforceError,fail,requireWorkforceAdmin,currentGrants,teamAccess,eligible,audit,capacityCount} from '../services/workforce';
...
router.get('/my-assignments',handle(async(req,res)=>{
  ...
  const rows=await db.select({id:assignments.id,status:assignments.status,cancellationReason:assignments.cancellationReason,role:shifts.role,station:shifts.station,startAt:shifts.startAt,endAt:shifts.endAt,breakMinutes:shifts.breakMinutes,
