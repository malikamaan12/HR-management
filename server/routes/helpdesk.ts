...
router.get('/config',handle(async(req,res)=>{res.json({canWorkQueue:helpdeskResponder(req.user!.role),confidentialTriage:helpdeskTriage(req.user!.role,true),attachmentsAvailable:privateStorageConfigured()});}));
...
router.post('/cases',upload,handle(async(req,res)=>{
  ...
  const result=await withUploads(uploaded=>db.transaction(async tx=>{
    const [row]=await tx.insert(cases).values({title:input.title,category:input.category,confidential:input.confidential||input.category==='employee_relations',requesterId:req.user!.userId}).returning();
    const [message]=await tx.insert(messages).values({caseId:row.id,authorId:req.user!.userId,body:input.body}).returning();
    await saveAttachment(tx,req,row.id,message.id,uploaded);await caseEvent(tx,req.user!,row.id,'Case opened');return {id:row.id};
...
router.post('/cases/:id/messages',upload,handle(async(req,res)=>{
  ...
  const result=await withUploads(uploaded=>db.transaction(async tx=>{
    ...
    await tx.update(cases).set({version:row.version+1,updatedAt:new Date(),status}).where(eq(cases.id,id));
...
router.post('/cases/:id/actions',handle(async(req,res)=>{
  ...
  await db.transaction(async tx=>{
    ...
    if(input.action==='assign'){
      ...
      updates.status=input.status;await caseEvent(tx,req.user!,id,`Status changed to ${statusLabels[input.status]}: ${input.reason}`);
    }else{
