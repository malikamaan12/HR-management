...
router.get('/corrections',async(req,res)=>{try{
 if(!reviewer(req.user!.role))throw new WorkflowError(403,'HR reviewer access is required');
...
router.get('/:id/corrections',async(req,res)=>{try{
 ...
 if(!employee||(!reviewer(req.user!.role)&&employee.userId!==req.user!.userId))throw new WorkflowError(404,'Employee corrections not found');
 const page=z.coerce.number().int().min(1).max(10000).default(1).parse(req.query.page);
...
router.post('/:id/corrections',async(req,res)=>{try{
 ...
 const result=await db.transaction(async tx=>{
  ...
  if(!employee)throw new WorkflowError(404,'Linked employee record not found');
  if(employee.recordVersion!==input.expectedVersion)throw new WorkflowError(409,'Employee record changed; reload before requesting a correction');
  const patch=Object.fromEntries(Object.entries(input.patch).filter(([k,v])=>v!==employee[k as keyof typeof employee]));
  if(!Object.keys(patch).length)throw new WorkflowError(400,'Choose at least one changed value');
...
router.post('/:id/corrections/:correctionId/decision',async(req,res)=>{try{
 ...
 const result=await db.transaction(async tx=>{
  ...
  if(!employee||(!reviewer(req.user!.role)&&employee.userId!==req.user!.userId))throw new WorkflowError(404,'Employee correction not found');
  const rows=await tx.execute(sql`SELECT * FROM employee_corrections WHERE id=${requestId} AND employee_id=${employeeId} FOR UPDATE`);const request=rows.rows[0];if(!request)throw new WorkflowError(404,'Correction not found');
  if(request.status!=='pending')throw new WorkflowError(409,'Correction is already decided');
  if(input.action==='withdraw'){if(request.requester_id!==req.user!.userId)throw new WorkflowError(403,'Only the requester can withdraw');}
  else if(!reviewer(req.user!.role)||request.requester_id===req.user!.userId||employee.userId===req.user!.userId)throw new WorkflowError(403,'An independent HR reviewer must decide this correction');
  if(input.action==='approve'){
   if(employee.recordVersion!==request.employee_version)throw new WorkflowError(409,'Employee record changed; reject or withdraw this request and submit a fresh correction');
