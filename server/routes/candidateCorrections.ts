...
import {recordHandler as handle,recordHistory} from '../services/workplaceRecords';
...
router.get('/candidates/:id/corrections',authenticate,handle(async(req,res)=>{
 ...
 const r=await db.execute(sql`SELECT version,reason,created_at,snapshot FROM lifecycle_history WHERE kind='candidate_correction' AND record_id=${candidateId} ORDER BY version DESC LIMIT 21 OFFSET ${(page-1)*20}`);
...
router.post('/candidates/:id/corrections',authenticate,handle(async(req,res)=>{
 ...
 const saved=await db.transaction(async tx=>{
  ...
  await tx.execute(sql`INSERT INTO lifecycle_history(kind,record_id,version,snapshot,actor_id,reason) VALUES ('candidate_correction',${candidateId},${old.recordVersion},${JSON.stringify(old)}::jsonb,${req.user.userId},'Prior candidate state before controlled correction') ON CONFLICT DO NOTHING`);
