...
import {z} from 'zod';
import {db} from '../db';
import {employees,users,workforceMembers as members,workforceMemberChanges as changes,workforceTeams as teams,workforceAssignments as assignments,
...
import {audit,eligible,fail,lockEmployee,requireWorkforceAdmin,teamAccess,WorkforceError,type WorkforceTransaction} from '../services/workforce';
...
router.get('/assignments/:id/presence',handle(async(req,res)=>{
  ...
  res.json(await db.transaction(async tx=>{
    ...
    return {presence:recorded?{...recorded,policySnapshot:undefined}:null,rules:policy.rules,opensAt:new Date(+shift.startAt-policy.rules.earlyMinutes*60000),closesAt:new Date(Math.min(+shift.endAt,+shift.startAt+policy.rules.lateMinutes*60000))};
  }));
}));
router.post('/assignments/:id/arrival',handle(async(req,res)=>{
  const id=positiveId.parse(req.params.id);z.object({}).strict().parse(req.body);
  res.json(await db.transaction(async tx=>{
    ...
    const [row]=await tx.insert(presence).values({assignmentId:id,employeeId:assignment.employeeId,arrivedAt:now,arrivedBy:req.user!.userId,policySnapshot:policy,flags:now>shift.startAt?['Arrival after scheduled start']:[]}).returning(presenceFields);
    await audit(tx,req.user!,'presence',row.id,'Employee recorded arrival');return row;
  }));
}));

async function depart(tx:WorkforceTransaction,user:TokenPayload,assignment:typeof assignments.$inferSelect,shift:typeof shifts.$inferSelect,reason?:string,version?:number) {
  ...
  const [saved]=await tx.update(presence).set({departedAt:now,departedBy:user.userId,departureReason:reason||null,flags}).where(eq(presence.id,row.id)).returning(presenceFields);
...
router.post('/assignments/:id/departure',handle(async(req,res)=>{
  const id=positiveId.parse(req.params.id);z.object({}).strict().parse(req.body);
  res.json(await db.transaction(async tx=>{const {assignment,shift}=await ownAssignment(tx,req.user!,id,true);return depart(tx,req.user!,assignment,shift);}));
}));
router.post('/presence/:id/review',handle(async(req,res)=>{
  const id=positiveId.parse(req.params.id),input=z.object({version:positiveId,action:z.enum(['close','review']),reason:operationReason}).strict().parse(req.body);
  res.json(await db.transaction(async tx=>{
    ...
    const [saved]=await tx.update(presence).set({reviewedAt:new Date(),reviewedBy:req.user!.userId,reviewNote:input.reason}).where(eq(presence.id,id)).returning(presenceFields);
    await audit(tx,req.user!,'presence',id,'Visit reviewed; payroll time requires separate approval');return saved;
  }));
}));

async function operationsAccess(tx:WorkforceTransaction,user:TokenPayload,shift:typeof shifts.$inferSelect) {
  try {const team=await teamAccess(tx,user,shift.teamId,'schedule',shift);return {canManage:true,team};}
  ...
  return {canManage:false,team:null};
}
router.get('/shifts/:id/operations',handle(async(req,res)=>{
  ...
  res.json(await db.transaction(async tx=>{
    ...
    return {canManage:access.canManage,incidents:rows.map(r=>({...r,history:history.filter(h=>h.incidentId===r.id)})),presence:await tx.select({...presenceFields,name,canReview:sql<boolean>`${access.canManage} and ${presence.arrivedBy} <> ${req.user!.userId} and (${employees.userId} is null or ${employees.userId} <> ${req.user!.userId})`}).from(presence).innerJoin(assignments,eq(presence.assignmentId,assignments.id)).innerJoin(employees,eq(assignments.employeeId,employees.id))
      .where(and(eq(assignments.shiftId,id),access.canManage?undefined:eq(employees.userId,req.user!.userId))).orderBy(presence.arrivedAt)};
  }));
}));
