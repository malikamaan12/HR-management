...
import {recordHandler as handle,recordHistory,requireAdmin,isAdmin} from '../services/workplaceRecords';
...
router.get('/policy/history',handle(async(req,res)=>{requireAdmin(req);const p=page.parse(req.query.page);const r=await db.execute(sql`SELECT version,reason,created_at,snapshot FROM lifecycle_history WHERE kind='reporting_policy' ORDER BY version DESC LIMIT 21 OFFSET ${(p-1)*20}`);res.json({items:r.rows.slice(0,20),hasMore:r.rows.length>20});}));
