...
import {recordHandler as handle} from '../services/workflowRecords';
...
router.get('/job/:jobId/history',handle(async(req,res)=>{
 ...
 const r=await db.execute(sql`SELECT version,actor_id,created_at,reason,snapshot FROM hr_workflow_history WHERE kind='employee_import' AND record_id=${job.id} ORDER BY version DESC LIMIT 21 OFFSET ${(page-1)*20}`);res.json({items:r.rows.slice(0,20),hasMore:r.rows.length>20});
}));
