...
Render Free runs the application against Supabase Free PostgreSQL with the Data API disabled and verified TLS. Private Supabase storage upload, signed download, public-access denial and cleanup were verified. The first administrator is active, bootstrap credentials were removed, and the approved private deployment repository is e3qatech/HR-management. Hosting remains at $0. Resend delivery is still pending sender configuration. Browser file attachment remains to be validated separately. See [the deployment guide](docs/Deployment-Guide.md).
