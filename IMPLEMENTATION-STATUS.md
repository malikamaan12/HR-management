...
The production Render Blueprint now defines a Node service and a new private PostgreSQL database, with migration execution, generated signing secrets, explicit build dependencies, and a database readiness endpoint. The public URL is validated and can use Render's assigned HTTPS origin. The full migration journal was tested on an empty PGlite database and rerun without duplicate application. Hosting publication, managed PostgreSQL verification, first-administrator setup, and live R2/Resend verification remain pending. See [the deployment guide](docs/Deployment-Guide.md).
