...
Latest local batch: daily and assigned-event-shift pay now map from compensation packages into payroll, with admin-configurable overtime and approved unpaid-leave deductions. Calculation evidence is visible on payroll details, and changed leave requires reconciliation before submission, approval or payment. TypeScript/build and 49 focused tests passed. This batch is not pushed or deployed; the live baseline below remains `b199199`. See [the payroll batch record](docs/Payroll-Units-Leave-Release.md).
