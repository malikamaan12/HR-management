...
3. The build installs locked dependencies and builds client/server. Each start runs `pnpm db:migrate && pnpm start`. All migrations through `0026_report_snapshots_candidate_corrections` must succeed before serving. The journal has passed empty-database, safe-rerun and prior-live upgrade tests, but failed real migrations still need investigation. Do not substitute `db:push`. Existing databases need backup and reconciliation first.
