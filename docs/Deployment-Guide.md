...
3. The build installs locked dependencies and builds client/server. Each start runs `pnpm db:migrate && pnpm start`. All migrations through `0014_document_renewal_requests` must succeed before serving. The journal has passed a safe-rerun test, but failed real migrations still need investigation. Do not substitute `db:push`. Existing databases need backup and reconciliation first.
