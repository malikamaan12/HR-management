...
## Combined operational release — 16 September 2026

The group rules remain active alongside HR Rules. Group attendance/timesheet rounding and break treatment are saved on records. Leave counts apply the configured count method and holiday exclusions together with the employee work calendar; HR Rules supplies entitlements, accrual and approvers. Payroll generation selects the group rounding policy at the pay-period start; reviewed adjustments preserve that snapshot. Cancelling and regenerating creates a fresh snapshot with retained history. Manual attendance overwrites and unreviewed payroll amount edits are replaced by the correction and pay-run review workflows.
