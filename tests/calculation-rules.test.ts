...
import {users,employees,calculationRuleVersions,hrRules} from '../shared/schema';
...
import attendanceOperationsRouter from '../server/routes/attendance-operations';
...
beforeAll(async()=>{
 ...
 context.db=drizzle(pg);const app=express();app.use(express.json());app.use('/settings',settingsRouter);app.use('/leaves',leaveRouter);app.use('/payroll',payrollRouter);app.use('/attendance',attendanceRouter);app.use('/attendance-operations',attendanceOperationsRouter);
...
test('leave uses the scoped authoritative policy, holidays and saved totals after rule changes',async()=>{
 ...
 await context.db.insert(hrRules).values({kind:'leave',name:'annual',employeeId:null,effectiveFrom:today(),createdBy:u.id,reason:'Synthetic entitlement policy',config:{paid:true,balanceRequired:false,accrualMode:'none',annualDays:0,monthlyDays:0,carryoverLimit:0,minServiceDays:0,maxConsecutiveDays:30,approverId:null}});
 ...
 expect((await preview(person.id)).body.totalDays).toBe(5);
});

test('clock-in and independently approved corrections preserve their calculation snapshots',async()=>{
 const admin=await account(),owner=await account('employee'),person=await employee(1,owner.id);
 let date=addDays(today(),-1);while(new Date(date).getUTCDay()!==1)date=addDays(date,-1);
 ...
 const version={body:(await context.db.insert(calculationRuleVersions).values({scope:'management_office',effectiveFrom:date,rules,createdBy:admin.id,reason:'Previously effective attendance policy'}).returning())[0]};const start=await clockAttendance(owner.id,'in',undefined,undefined,time('09:12'));expect(start.status).toBe('late');
 const changed=fresh();changed.attendance.breakTreatment='paid';await context.db.insert(calculationRuleVersions).values({scope:'management_office',effectiveFrom:date,rules:changed,createdBy:admin.id,reason:'Updated attendance policy'});
 ...
 const correction=await request(owner.token,'/attendance-operations/corrections',{employeeId:person.id,date,expectedVersion:ended.version,checkIn:time('09:12').toISOString(),checkOut:time('17:03').toISOString(),breakMinutes:30,reason:'Correct recorded end time'});expect(correction.status).toBe(201);
 expect((await request(owner.token,'/attendance-operations/corrections/'+correction.body.id+'/review',{decision:'approved',reason:'Cannot approve own record'})).status).toBe(404);
 expect((await request(admin.token,'/attendance-operations/corrections/'+correction.body.id+'/review',{decision:'approved',reason:'Verified corrected end time'})).status).toBe(200);
 const saved=await request(admin.token,'/attendance-operations/day/'+person.id+'/'+date);expect(saved.body.record.totalWorkHours).toBe(435);expect(saved.body.record.calculationSnapshot.version).toBe(version.body.id);
});

test('generated payroll pins period rounding through later policy revisions and adjustments',async()=>{
 const u=await account(),reviewer=await account('super_admin','reviewer'),person=await employee(1),second=await employee(2);
 const payment=new Date(today()+'T00:00:00Z'),periodStart=new Date(Date.UTC(payment.getUTCFullYear(),payment.getUTCMonth()-1,1)).toISOString().slice(0,10);
 const rules=fresh();rules.payroll={roundingCents:10,roundingMode:'up'};
 const [version]=await context.db.insert(calculationRuleVersions).values({scope:'management_office',effectiveFrom:periodStart,rules,createdBy:u.id,reason:'Existing effective payroll calculation'}).returning();
 await context.db.insert(hrRules).values({kind:'payroll',name:'Pay policy',employeeId:null,effectiveFrom:periodStart,createdBy:u.id,reason:'Existing effective pay policy',config:{currency:'QAR',cycleStartDay:1,payDay:5,basis:'salary',basicSalary:'1000.03',hourlyRate:'10.00',regularMinutesPerDay:480,overtimeMultiplier:1.5,allowances:{},deductions:{},approverId:reviewer.id}});
 const input={employeeId:person.id,year:payment.getUTCFullYear(),month:payment.getUTCMonth()+1};
 ...
 const changed=fresh();changed.payroll={roundingCents:100,roundingMode:'down'};
 await context.db.insert(calculationRuleVersions).values({scope:'management_office',effectiveFrom:periodStart,rules:changed,createdBy:u.id,reason:'Later imported effective rule'});
 const edited=await request(u.token,'/payroll/'+draft.body.id,{version:draft.body.review.version,adjustments:[{label:'Correction',kind:'allowance',amount:'0.01',reason:'Verified adjustment'}],reason:'Correct allowance'},'PATCH');expect(edited.status).toBe(200);
 const saved=await request(u.token,'/payroll/'+draft.body.id);expect(saved.body.record.netSalary).toBe('1000.10');expect(saved.body.record.roundingAdjustmentCents).toBe(6);expect(saved.body.record.calculationSnapshot.version).toBe(version.id);
