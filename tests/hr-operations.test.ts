...
test('a failed leave audit rolls back its request, reservation and newly posted accrual', async () => {
    const f = await fixture();
    await rule(f, 'leave', leaveConfig);
    await pg.exec("CREATE FUNCTION reject_leave_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.entity_type='leave' THEN RAISE EXCEPTION 'forced audit failure'; END IF; RETURN NEW; END $$; CREATE TRIGGER reject_leave_audit BEFORE INSERT ON activity_logs FOR EACH ROW EXECUTE FUNCTION reject_leave_audit();");
    try {
