...
beforeEach(async()=>{await pg.exec('TRUNCATE employees,users,lifecycle_history,app_settings RESTART IDENTITY CASCADE');files.upload.mockReset().mockResolvedValue('private/proposal.pdf');files.remove.mockReset().mockResolvedValue(undefined);files.download.mockReset().mockResolvedValue('https://files.example.test/private');});
...
test('history failure rolls back policy, assignment and approved document changes without deleting the proposal',async()=>{
 const f=await fixture(),d=await doc(f.alice),r=await submit(f,d);
 await pg.exec("CREATE FUNCTION fail_document_history() RETURNS trigger AS $$ BEGIN RAISE EXCEPTION 'test'; END; $$ LANGUAGE plpgsql; CREATE TRIGGER fail_document_history BEFORE INSERT ON lifecycle_history FOR EACH ROW EXECUTE FUNCTION fail_document_history();");
 try{
 ...
 }finally{await pg.exec('DROP TRIGGER fail_document_history ON lifecycle_history; DROP FUNCTION fail_document_history();');}
});
