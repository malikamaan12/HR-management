...
test.each([25,26,27,28,38,41,43])('upgrades live journal through migration %i without changing existing records',async(cutoff)=>{
 ...
 try{
  ...
  if(cutoff===43)await pg.exec("INSERT INTO reporting_policies(version,turnover_denominator,created_by) VALUES(1,'opening',1); INSERT INTO report_runs(owner_id,request_key,report_type,filters,snapshot) VALUES(1,'e1d97c36-fd80-4d6e-913d-749c2c54f6a4','headcount','{}','{\"retained\":true}')");
  await migrate(db,{migrationsFolder:current});
  if(cutoff===43){expect((await pg.query('SELECT snapshot,access_stamp FROM report_runs')).rows[0]).toEqual({snapshot:{retained:true},access_stamp:null});expect((await pg.query('SELECT turnover_denominator,config FROM reporting_policies')).rows[0]).toEqual({turnover_denominator:'opening',config:{maxRows:5000,minimumPerformanceSample:5,schedulesEnabled:true}});}
