...
import reports from '../server/routes/reporting';
...
const run=(u:any,kind='headcount',filters:any={},requestKey=randomUUID())=>req(u,'/reporting/runs',{kind,filters,requestKey});
...
const correction=(u:any,c:any,fields:any,expectedVersion=c.recordVersion)=>req(u,`/candidates/${c.id}/corrections`,{fields,expectedVersion,reason});
beforeAll(async()=>{process.env.JWT_SECRET='report-corrections-access-secret-32-characters';process.env.JWT_REFRESH_SECRET='report-corrections-refresh-secret-32-characters';pg=new PGlite();for(const f of readdirSync(new URL('../migrations',import.meta.url)).filter(n=>n.endsWith('.sql')).sort())await pg.exec(readFileSync(new URL('../migrations/'+f,import.meta.url),'utf8'));ctx.db=drizzle(pg);const app=express();app.use(express.json());app.use(authenticate,moduleAccess);app.use('/reporting',reports);app.use(corrections);server=app.listen(0,'127.0.0.1');await new Promise<void>(r=>server.once('listening',r));base='http://127.0.0.1:'+(server.address() as any).port;});
beforeEach(async()=>{await pg.exec('TRUNCATE employees,users,candidates,workforce_sites,lifecycle_history,app_settings RESTART IDENTITY CASCADE');});
...
test('report snapshots require both report and underlying module access and remain owner-private',async()=>{
 ...
 expect((await req(null,'/reporting/catalog')).status).toBe(401);expect((await run(staff)).status).toBe(403);expect((await run(exec,'compliance')).status).toBe(403);
 const r=await run(admin);expect(r.status).toBe(201);expect(r.headers.get('cache-control')).toBe('no-store');
 for(const suffix of ['','.csv'])expect((await req(other,`/reporting/runs/${r.body.id}${suffix}`)).status).toBe(404);
 expect((await req(other,'/reporting/runs')).body.items).toHaveLength(0);
 await ctx.db.update(s.users).set({role:'employee'}).where(eq(s.users.id,admin.id));expect((await req(admin,`/reporting/runs/${r.body.id}.csv`)).status).toBe(403);
});
test('saved totals and CSV remain stable after data changes; retry keys are bound to filters',async()=>{
 ...
 const csv=await req(u,`/reporting/runs/${r.body.id}.csv`);expect(csv.status).toBe(200);expect(csv.body).toContain("' =SUM(1,2)");expect(csv.body).not.toContain('PRIVATE');expect((await req(u,`/reporting/runs/${r.body.id}`)).body.snapshot.summary.registered_employees).toBe(1);
});
test('turnover uses historical date endpoints and admin policy is pinned to each run',async()=>{
 ...
 const input={version:0,turnoverDenominator:'opening',reason};expect((await req(hr,'/reporting/policy',input)).status).toBe(403);expect((await req(u,'/reporting/policy',input)).status).toBe(201);expect((await req(u,'/reporting/policy',input)).status).toBe(409);
 expect((await run(u,'turnover',filters)).body.snapshot.summary.turnover_percent).toBe(50);expect((await req(u,`/reporting/runs/${r.body.id}`)).body.snapshot.policy.version).toBe(0);expect((await req(u,'/reporting/policy/history')).body.items).toHaveLength(1);
});
test('leave sums saved approved booking days rather than calendar duration or overlapping bookings',async()=>{
 const u=await user(),e=await employee();await ctx.db.insert(s.leaves).values([{employeeId:e.id,leaveType:'annual',startDate:'2026-01-30',endDate:'2026-02-05',totalDays:2.5,reason:'Test',status:'approved'},{employeeId:e.id,leaveType:'annual',startDate:'2025-12-30',endDate:'2026-01-05',totalDays:4,reason:'Test',status:'approved'},{employeeId:e.id,leaveType:'annual',startDate:'2026-01-10',endDate:'2026-01-11',totalDays:1,reason:'Test',status:'pending'}]);
 const r=await run(u,'leave',{from:'2026-01-01',to:'2026-01-31'});expect(r.status).toBe(201);expect(r.body.snapshot.summary).toEqual({bookings:1,charged_days:2.5});
});
...
test('invalid filters reject and failed audit rolls back a report run',async()=>{
 ...
 await pg.exec("ALTER TABLE activity_logs ADD CONSTRAINT fail_report_audit CHECK(entity_type <> 'report_run')");try{expect((await run(u)).status).toBe(500);expect((await req(u,'/reporting/runs')).body.items).toHaveLength(0);}finally{await pg.exec('ALTER TABLE activity_logs DROP CONSTRAINT fail_report_audit');}
 expect((await req(u,'/reporting/export/headcount')).status).toBe(410);
});
test('candidate corrections preserve versions, reject forged fields, and keep private history',async()=>{
 const u=await user(),staff=await user('staff','employee'),c=await candidate();expect((await correction(staff,c,{email:'new@example.test'})).status).toBe(403);expect((await req(staff,`/candidates/${c.id}/corrections`)).status).toBe(403);
 ...
 const h=await req(u,`/candidates/${c.id}/corrections`);expect(h.body.items.map((r:any)=>r.version)).toEqual([2,1]);expect(h.body.items[1].snapshot.fullNameEn).toBe('Initial Candidate');expect((await ctx.db.select().from(s.candidates))).toHaveLength(1);
});
test('candidate identity freezes at first offer, contact corrections remain possible, and SQL guards reject bypasses',async()=>{
 ...
 expect((await req(u,`/candidates/${c.id}/correction-record`)).body.identityLocked).toBe(true);expect((await correction(u,c,{fullNameEn:'Different Person'})).status).toBe(409);await expect(pg.query('UPDATE candidates SET full_name_en=$1 WHERE id=$2',['Different Person',c.id])).rejects.toThrow('frozen');
