...
## Supabase database certificates and live storage verification

For Supabase's session pooler, download the official database CA from **Database Settings → SSL configuration → Download certificate**. On Render, save the certificate as Secret File `supabase-ca.crt`, then add `sslmode=verify-full&sslrootcert=%2Fetc%2Fsecrets%2Fsupabase-ca.crt` to DATABASE_URL. Keep the password URL-encoded. Verify the saved value after editing masked fields. Never solve TLS failures by disabling certificate or hostname verification. See [Supabase SSL enforcement](https://supabase.com/docs/guides/platform/ssl-enforcement).

Run `pnpm storage:verify` in the configured Supabase server environment to check real upload, byte-for-byte read, a signed attachment download, and rejection of public downloads. It creates one tiny synthetic PNG under a unique `deployment-checks/` key and removes only that object in cleanup. It prints neither credentials nor signed URLs. This checks the storage service; it does not replace application authorization tests or a complete user upload workflow.

If Free Render is the only available database-connected runtime, first-admin bootstrap can be run once using a temporary start command after owner approval: `pnpm db:migrate && pnpm admin:bootstrap && pnpm start`. Verify account creation before retrying. Restore the normal `pnpm db:migrate && pnpm start` command, remove all BOOTSTRAP_ADMIN variables, and redeploy immediately after success. Never leave bootstrap enabled for routine starts. The same temporary-command approach can run storage verification, followed by restoring normal startup.
