...
export const users = pgTable("users", {
  ...
  accountVersion: integer("account_version").default(1).notNull(),
...
export const lifecycleTemplates=pgTable('lifecycle_templates',{
 id:serial('id').primaryKey(),name:text('name').notNull(),kind:text('kind').notNull(),tasks:jsonb('tasks').$type<{title:string;kind:'general'|'document'|'asset_return';required:boolean;offsetDays:number;reviewRequired?:boolean;documentType?:string}[]>().notNull(),version:integer('version').notNull().default(1),active:boolean('active').notNull().default(true),createdBy:integer('created_by').notNull().references(()=>users.id),createdAt:timestamp('created_at',{withTimezone:true}).defaultNow().notNull(),
});
...
export const lifecycleTasks=pgTable('lifecycle_tasks',{
 version:integer('version').notNull().default(1),reviewRequired:boolean('review_required').notNull().default(false),reviewState:text('review_state').notNull().default('not_required'),documentType:text('document_type').notNull().default(''),documentSnapshot:jsonb('document_snapshot'),submittedBy:integer('submitted_by').references(()=>users.id),submittedAt:timestamp('submitted_at',{withTimezone:true}),reviewDueDate:date('review_due_date'),reviewedBy:integer('reviewed_by').references(()=>users.id),reviewedAt:timestamp('reviewed_at',{withTimezone:true}),reviewNote:text('review_note'),
