import { z } from 'zod';
import { categoryIdInput } from './helpdesk-workspace';
const id=z.coerce.number().int().positive();
export const helpdeskPolicyAdmin=(role:string)=>['super_admin','admin','hr_director','hr'].includes(role);
export const helpdeskPolicyInput=z.object({category:categoryIdInput,confidential:z.boolean(),employeeId:id.nullable(),enabled:z.boolean(),
...
export const articleInput=z.object({title:z.string().trim().min(4).max(200),body:z.string().trim().min(10).max(30000),category:categoryIdInput,
