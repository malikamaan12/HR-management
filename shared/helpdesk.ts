...
export const caseCategories=['payroll','attendance','documents','shifts','transport_accommodation','equipment','employee_relations','other'] as const;
export const categoryLabels={payroll:'Payroll query',attendance:'Attendance correction',documents:'Document or letter request',shifts:'Shift support',transport_accommodation:'Transport or accommodation',equipment:'Equipment or uniform',employee_relations:'Employee relations',other:'Other HR request'};
...
export const newCaseInput=z.object({title:z.string().trim().min(4).max(160),category:z.enum(caseCategories),confidential:booleanInput.default(false),body:z.string().trim().min(10).max(10000)}).strict();
